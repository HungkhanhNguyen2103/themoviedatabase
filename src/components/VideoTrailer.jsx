...
export default function VideoTraler(props){
    ...
    useEffect(()=>{
        ...
        if(props.item.key == null && props.site !== 'game'){
    ...
    return (
        <>
        {
            props.item.site === 'YouTube' ? (
                <div><iframe width={700} height={500} src={`https://www.youtube-nocookie.com/embed/${props.item.key}`} title={props.item.name} frameBorder={0} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen /></div>
